...
fn test_quantization_levels() {
    ...
    assert!(matches!(config.quantization, QuantizationLevel::Int4 { group_size: 32 }));
    ...
    assert!(matches!(config.quantization, QuantizationLevel::Int8 { group_size: 128 }));
...
fn test_cpu_target_optimization() {
    ...
    for target in targets {
        ...
        match target {
            CpuTarget::RaspberryPi4 => {
                assert!(matches!(config.quantization, 
...
fn test_cloud_config_integration() {
    ...
    let anthropic_config = CloudConfig::anthropic(
        "test-key".to_string(),
